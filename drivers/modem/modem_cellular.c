...
#if DT_HAS_COMPAT_STATUS_OKAY(quectel_m66)
MODEM_CHAT_SCRIPT_CMDS_DEFINE(quectel_m66_init_chat_script_cmds,
...
MODEM_CHAT_SCRIPT_DEFINE(quectel_m66_init_chat_script, quectel_m66_init_chat_script_cmds,
			 abort_matches, modem_cellular_chat_callback_handler, 10);

MODEM_CHAT_SCRIPT_CMDS_DEFINE(quectel_m66_dial_chat_script_cmds,
			      MODEM_CHAT_SCRIPT_CMD_RESP_MULT("AT+CGACT=0,1", allow_match),
			      MODEM_CHAT_SCRIPT_CMD_RESP("AT+CGDCONT=1,\"IP\","
							 "\""CONFIG_MODEM_CELLULAR_APN"\"",
							 ok_match),
...
MODEM_CHAT_SCRIPT_DEFINE(quectel_m66_dial_chat_script, quectel_m66_dial_chat_script_cmds,
			 dial_abort_matches, modem_cellular_chat_callback_handler, 10);

MODEM_CHAT_SCRIPT_CMDS_DEFINE(quectel_m66_periodic_chat_script_cmds,
			      MODEM_CHAT_SCRIPT_CMD_RESP("AT+CREG?", ok_match),
			      MODEM_CHAT_SCRIPT_CMD_RESP("AT+CGREG?", ok_match),
			      MODEM_CHAT_SCRIPT_CMD_RESP("AT+CSQ", csq_match));

MODEM_CHAT_SCRIPT_DEFINE(quectel_m66_periodic_chat_script,
			 quectel_m66_periodic_chat_script_cmds, abort_matches,
			 modem_cellular_chat_callback_handler, 4);
#endif
...
#define MODEM_CELLULAR_DEVICE_QUECTEL_M66(inst)                                                    \
	MODEM_PPP_DEFINE(MODEM_CELLULAR_INST_NAME(ppp, inst), NULL, 98, 1500, 64);                 \
                                                                                                   \
	static struct modem_cellular_data MODEM_CELLULAR_INST_NAME(data, inst) = {                 \
		.chat_delimiter = {'\r'},                                                          \
	...
	static struct modem_cellular_config MODEM_CELLULAR_INST_NAME(config, inst) = {             \
		...
		.shutdown_time_ms = 5000,                                                          \
		.init_chat_script = &quectel_m66_init_chat_script,                                 \
		.dial_chat_script = &quectel_m66_dial_chat_script,                                 \
		.periodic_chat_script = &quectel_m66_periodic_chat_script,                         \
	};                                                                                         \
                                                                                                   \
	PM_DEVICE_DT_INST_DEFINE(inst, modem_cellular_pm_action);                                  \
                                                                                                   \
...
#define DT_DRV_COMPAT quectel_m66
DT_INST_FOREACH_STATUS_OKAY(MODEM_CELLULAR_DEVICE_QUECTEL_M66)
#undef DT_DRV_COMPAT
